...
| `analyses/2026-05-04_predictor_v7_target_diagnosis.md` | **Stage A v7 (v12 labels) target-head 21 cm L2 disaster + v7-fix.** v7 retrain on v12 labels: target xyz L2 21.66 cm (vs v6 baseline 5-10 cm). Root causes: (A) legacy `gt_contact > 0.5` gating wastes ~50% target supervision (closest-surface-point xyz is well-defined for every cell, but v12 contact frac drops to 50%); (B) Kendall mis-adapted (log_var pushed weight to 23× yet L2 stayed 21 cm — small smooth-L1 numerical loss tricked the auto-balancer); (C) smooth-L1 gradient saturation. v7-fix (commit `32dc2b5`): Kendall off, target_weight=5.0, new `target_gate_kind="all"` PredictorLoss option supervises every (frame, part) cell. Predicted target L2 → 6-10 cm. Stage B v18 blocked until v7-fix acceptance (L2 < 12 cm). |
