...
#
# Includes (~50-80 MB):
#   - analyses/round28_<variant>_diag_{best_val,final}/*: same-train diagnostics
#   - analyses/round28_<variant>_heldout_val_diag_{best_val,final}/*: true-val diagnostics
...
#   - analyses/round28_*indices*.json / analyses/round28_*selection*.json
#   - analyses/round28_a_group_ablation_matrix_summary.{md,json}
...
for V in "${VARIANTS[@]}"; do
    for TAG in best_val final; do
        ...
        [[ -d "${DIR}" ]] && ITEMS+=("${DIR}")
        DIR="analyses/round28_${V}_heldout_val_diag_${TAG}"
        [[ -d "${DIR}" ]] && ITEMS+=("${DIR}")
    done
done
for DIR in \
...
for f in \
    ...
    analyses/round27_tier0_eval_selection_balanced.json \
    analyses/round28_val_tier0_indices_48_balanced.json \
    analyses/round28_val_tier0_eval_selection_balanced.json \
    analyses/round28_body_action_train_indices_48.json \
    analyses/round28_body_action_eval_selection.json \
    analyses/round28_body_action_val_indices_48.json \
    analyses/round28_body_action_val_eval_selection.json \
    analyses/round28_a_group_ablation_matrix_summary.md \
    analyses/round28_a_group_ablation_matrix_summary.json \
    analyses/round28_claude_code_stage2_oracle_interface_prompt.md
do
    [[ -f "${f}" ]] && ITEMS+=("${f}")
done
