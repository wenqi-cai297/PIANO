#!/usr/bin/env bash
# Round-32 V7 Stage-1.5 anti-bug ablation matrix — integrated train +
# downstream-coupling diag launcher.
#
# Baseline = stage1p5_interaction_v0 (the historical V0 ckpt). V7 tests
# 4 anti-bug mechanisms targeting the Phase 1 audit's findings:
#   V7-A : channel moment-match velocity (B1 + B2)
#   V7-B : phase unit-norm + angle consistency (B3)
#   V7-C : stance + walking BCE weight x4 (B4)
#   V7-D : C41 wrist frame-0 invariant (B5)
#
# Six variants — see analyses/round32_stage1p5_v7_manifest.md.
#
# For each variant: TRAIN (from scratch, 80 ep, bs=48) -> R32 B
# downstream-coupling diag against frozen PB1 with oracle Stage-1 cond
# (val bucket only by default) -> archive diag under
# analyses/round32_v7_diag_<variant>/.
#
# **GPU restriction**: defaults to CUDA_VISIBLE_DEVICES=0,2 (matches V7
# and V8 R31 launchers). Override with ROUND32_V7_GPUS.
#
# Total time for all 6 variants: ~2.5 h (~25 min / variant — Stage-1.5
# is bigger than Stage-1 so each takes a bit longer than the R31 V7 18
# min / variant).
#
# Usage:
#   tmux new -s r32v7
#   export DATASETS_ROOT=/media/8TB_data/Cai/datasets/InterAct/piano_official_process_4
#   bash scripts/stage_a_generator/run_round32_v7_matrix.sh
#
#   # Subset:
#   bash scripts/stage_a_generator/run_round32_v7_matrix.sh \
#       --only stage1p5_v7_v0_control,stage1p5_v7_v1_moment,stage1p5_v7_v5_full
#
#   # Dry-run:
#   bash scripts/stage_a_generator/run_round32_v7_matrix.sh --dry-run
#
# Environment overrides:
#   ROUND32_V7_GPUS="0,2"          CUDA_VISIBLE_DEVICES mask
#   ROUND32_V7_NUM_PROCESSES=N      accelerate --num_processes
#   ROUND32_V7_BUCKETS="val"        diag buckets
#   ROUND32_V7_ALLOW_PARTIAL=1      keep going on a per-variant failure
...
FORCE_RETRAIN=0
FORCE_REDIAG=0
ALLOW_PARTIAL="${ROUND32_V7_ALLOW_PARTIAL:-0}"
BUCKETS_STR="${ROUND32_V7_BUCKETS:-val}"
...
GPUS="${ROUND32_V7_GPUS:-0,2}"
export CUDA_VISIBLE_DEVICES="${GPUS}"

NUM_GPUS_IN_MASK="$(echo "${GPUS}" | tr ',' '\n' | grep -c '^[0-9]\+$' || true)"
if [[ "${NUM_GPUS_IN_MASK}" -lt 1 ]]; then NUM_GPUS_IN_MASK=1; fi

: "${ROUND32_V7_NUM_PROCESSES:=${NUM_GPUS_IN_MASK}}"
export ROUND32_V7_NUM_PROCESSES
...
OVERALL_LOG_DIR="runs/round32_v7_matrix"
...
log "[V7] Regenerating Stage-1.5 V7 ablation configs."
if [[ ${DRY_RUN} -eq 0 ]]; then
    "${PY}" scripts/stage_a_generator/round32_make_stage1p5_v7_configs.py \
        --data-root "${DATASETS_ROOT}" 2>&1 | tee -a "${SUMMARY_LOG}"
fi

MANIFEST="analyses/round32_stage1p5_v7_manifest.json"
...
if [[ ${DRY_RUN} -eq 1 && ! -f "${MANIFEST}" ]]; then
    ...
    VARIANTS="stage1p5_v7_v0_control configs/training/stage1p5_v7_v0_control.yaml runs/training/stage1p5_v7_v0_control
stage1p5_v7_v1_moment configs/training/stage1p5_v7_v1_moment.yaml runs/training/stage1p5_v7_v1_moment
stage1p5_v7_v2_phase configs/training/stage1p5_v7_v2_phase.yaml runs/training/stage1p5_v7_v2_phase
stage1p5_v7_v3_stance configs/training/stage1p5_v7_v3_stance.yaml runs/training/stage1p5_v7_v3_stance
stage1p5_v7_v4_frame0 configs/training/stage1p5_v7_v4_frame0.yaml runs/training/stage1p5_v7_v4_frame0
stage1p5_v7_v5_full configs/training/stage1p5_v7_v5_full.yaml runs/training/stage1p5_v7_v5_full"
else
...
log
log "===== R32 V7 ablation matrix launch ${STAMP} ====="
log "DATASETS_ROOT=${DATASETS_ROOT}"
log "GPUS (CUDA_VISIBLE_DEVICES)=${GPUS}  NUM_PROCESSES=${ROUND32_V7_NUM_PROCESSES}"
...
while IFS=' ' read -r VID CFG OUTDIR; do
    ...
    DIAG_ARCHIVE="analyses/round32_v7_diag_${VID}"
    ...
    if [[ ${SKIP_TRAIN} -eq 0 ]]; then
        ...
        if [[ -f "${FINAL}" && ${FORCE_RETRAIN} -eq 0 ]]; then
            ...
            log "    output: ${OUTDIR}"
            log "    log:    ${VARIANT_LOG}"
            log "    GPUs:   ${GPUS}  procs=${ROUND32_V7_NUM_PROCESSES}"
            log "================================================================"

            if [[ "${ROUND32_V7_NUM_PROCESSES}" -le 1 ]]; then
                TRAIN_CMD=("${PY}" -u src/piano/training/train_stage1p5.py --config "${CFG}")
            else
                TRAIN_CMD=(accelerate launch
                    --num_processes "${ROUND32_V7_NUM_PROCESSES}"
    ...
    if [[ ${SKIP_DIAG} -eq 0 ]]; then
        ...
        # Per-variant OUT_TAG so diag output dirs don't collide.
        DS_OUT_TAG="_v7_${VID}"
...
SUMMARY_MD="analyses/round32_v7_matrix_summary_${STAMP}.md"
...
rows = []
for v in variants:
    base = Path(f"analyses/round32_v7_diag_{v}")
...
out = [
    "# R32 V7 Stage-1.5 anti-bug ablation matrix — comparison summary (val)",
    "",
    f"Stamp: {stamp}",
    "",
    "Reference baselines (Stage-1 oracle + V0 Stage-1.5 -> PB1):",
    "| metric | A oracle (full GT cond) | V0 (Stage-1.5 V0) | ship gate |",
    "|---|---:|---:|---|",
    "| drift_max mean (cm) | 7.55 | 15.72 | regression <= +1 cm vs oracle |",
    "| left_hand drift_max (cm) | (PB1 oracle ~10 cm) | 22.83 | wrist signal target |",
    "| right_hand drift_max (cm) | (PB1 oracle ~10 cm) | 26.68 | wrist signal target |",
    "| L_R_corr | -0.219 | (V0 audit baseline) | regression <= +0.05 |",
    "| step_period_rate | 39.0 % | (V0 audit baseline) | regression <= -3 pp |",
    "| lw dir_cos | 0.903 | (V0 audit baseline) | — |",
    "| rw dir_cos | 0.912 | (V0 audit baseline) | — |",
    "",
    "## Per-variant numbers",
    "",
...
out.append("")
out.append("- V0 control should ~= the historical V0 audit (lh ~22, rh ~27).")
out.append("- V5 (full) closes >= 6 cm wrist drift over V0 -> ship V5, queue cascade")
out.append("  training (V8) on top.")
out.append("- V1 (moment) alone closes >= 4 cm -> V7-A dominant; ship V1.")
out.append("- V4 (frame0) alone closes >= 3 cm -> B5 was dominant; ship V4 + queue rest.")
out.append("- All V7.* close < 2 cm -> per-channel loss design insufficient; pivot")
out.append("  to cascade training (Stage-1.5 learns to consume Stage-1 generated cond).")
...
# ─── Phase 4: Pack ─────────────────────────────────────────────────
if [[ ${DRY_RUN} -eq 0 ]]; then
    TARBALL="analyses/round32_v7_matrix_results_${STAMP}.tar.gz"
    log
    log "================================================================"
    ...
    for VID in "${DIAGED_OK_VIDS[@]}"; do
        A="analyses/round32_v7_diag_${VID}"
        [[ -d "${A}" ]] && PACK_TARGETS+=("${A}")
    done
    [[ -f "${SUMMARY_MD}" ]] && PACK_TARGETS+=("${SUMMARY_MD}")
    [[ -f "${SUMMARY_LOG}" ]] && PACK_TARGETS+=("${SUMMARY_LOG}")
    [[ -f "${MANIFEST}" ]] && PACK_TARGETS+=("${MANIFEST}")
    [[ -f analyses/round32_stage1p5_v7_manifest.md ]] && PACK_TARGETS+=("analyses/round32_stage1p5_v7_manifest.md")
...
log
log "================================================================"
log "[$(date '+%F %T')] R32 V7 matrix COMPLETE"
log "================================================================"
