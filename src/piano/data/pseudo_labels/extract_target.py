...
def extract_contact_target(
    ...
    patch_centers : (K, 3) — the patch center positions (for later use)
    """
    if config is None:
        config = TargetConfig()
    ...
    # Compute patch centers via FPS on object surface
    ...
    # Build KD-tree for fast nearest-patch lookup
    tree = build_kdtree(patch_centers)
    ...
    for bp_idx, joint_idx in enumerate(BODY_PART_INDICES):
        ...
        for t in range(T):
            if contact_state[t, bp_idx] < config.contact_threshold:
                # No contact — leave as uniform zero (or could set uniform)
                continue

            # Soft assignment to nearest patches
